...
public class AVL<T extends Comparable<T>> {
	...
	public void get_list(LinkedList<T> sorted){
		printHelper(this.root,sorted);
	}
	
	private void printHelper(AVLNode<T> temp,LinkedList<T> sorted){
		if(temp == null)return;
		
		printHelper(temp.getLeft(),sorted);
		sorted.add(temp.getdata());
		printHelper(temp.getRight(),sorted);
